...
const ConnectIcon = chakra(ArrowTopRightOnSquareIcon, iconProps);
...
const ActionButtons: FC<ActionButtonsProps> = ({ user }) => {
  ...
  return (
    <HStack
      ...
      gap={0}
    >
      <Tooltip label="Open in VPN App" placement="top">
        <IconButton
          p="0 !important"
          aria-label="connect"
          bg="transparent"
      ...
      <CopyToClipboard text={subUrl} onCopy={() => { setCopied([0, true]); }}>
        <div>
          <Tooltip
      ...
      <Tooltip label="QR Code" placement="top">
        <IconButton
          ...
          _dark={{ _hover: { bg: "gray.700" } }}
          size={{ base: "sm", md: "md" }}
          onClick={() => { setQRCode(user.links); setSubLink(user.subscription_url); }}
        >
          <QRIcon />
        </IconButton>
      </Tooltip>
